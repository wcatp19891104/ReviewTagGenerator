...
class Trie(object):
    ...
    @cherrypy.expose
    def autocomplete(self, prefix, max_results = MAX_AUTOCOMPLETION):
        ...
        return json.dumps(list(tup[0] for tup in sorted(weighted_suffixes.items(), key = operator.itemgetter(1), reverse=True)[:max_results]))
