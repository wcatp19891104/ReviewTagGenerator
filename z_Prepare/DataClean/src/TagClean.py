...
from Prepare.DataClean.src import MyTagger
import TagProcessor
...
class TagClean:
    def __init__(self):
        ...
        self.cons = list()
        self.pros = list()
        self.valid_words = {l.strip() for l in codecs.open('../dictionary/allEnglishWords.txt', encoding='utf-8').readlines()}
        ...
        self.data = codecs.open('../resource/babiesrus_review.txt', encoding='utf-8').readlines()
        self.useful_field = {l.strip() for l in codecs.open('../dictionary/emotion_list.txt', encoding='utf-8').readlines() if l.strip()[0] != '#'}
        self.emotion_except = {l.strip() for l in codecs.open('../dictionary/neutral_word.txt', encoding='utf-8').readlines() if l.strip()[0] != '#'}
