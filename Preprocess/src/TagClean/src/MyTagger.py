...
UDT_TAG_FILE = CURR_DICT + "/../dictionary/UDF_Tagger.txt"


class MyTagger:
    def __init__(self):
        self.default_tagger = nltk.data.load(nltk.tag._POS_TAGGER)
        model = {l.strip().split('\t')[0]: l.strip().split('\t')[1] for l in codecs.open(UDT_TAG_FILE, encoding='utf-8')
            .readlines() if l.strip()[0] != '#'}
